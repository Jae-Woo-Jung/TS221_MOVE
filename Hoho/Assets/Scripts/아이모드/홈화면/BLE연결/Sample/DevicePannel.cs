...
public class DevicePannel : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            bool isConnected = AndroidBLEPluginStart._bleControlObj.Call<bool>("isConnected");
            var datas = AndroidBLEPluginStart.characteristicDatas;
            ...
            Android.Data.BleScannedDevice device = AndroidBLEPluginStart.scannedDevices.Find(x => (x.address == addr) && (x.name == DeviceName));
            if (device.name !=null)
            {
                setRssi(device.rssi);
            }
    ...
    private void ConnectBtnController()
    {
        ...
        {
            AndroidBLEPluginStart._bleControlObj.Call<string>("connectExternal", addr);
        }
        else
        {
            AndroidBLEPluginStart._bleControlObj.Call("disconnectGattServer");
        }
    }
